 ...
 * migrateToSaas.js
 *
 * One-time, idempotent migration that converts the single-tenant HEXAONE
 ...
 *   2. Create the HEXAONE tenant record (if not already present)
...
async function run() {
  ...
  // ── Step 3: Create HEXAONE tenant ──────────────────────────────────────
  console.log('\nStep 3: Creating HEXAONE tenant...');
  ...
  if (!zaneTenant) {
    ...
    console.log(`  ✓ Created HEXAONE tenant, id: ${zaneTenant.id}`);
  } else {
    console.log(`  - HEXAONE tenant already exists, id: ${zaneTenant.id}`);
  }
