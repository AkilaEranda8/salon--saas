...
# Trigger forgot-password for superadmin user via the API
cmd = (
    "echo '=== Testing forgot-password API ===' && "
    "curl -sk -X POST http://localhost:5001/api/auth/forgot-password "
    "-H 'Content-Type: application/json' "
    "-d '{\"username\":\"superadmin\"}' && echo && "
    "echo '=== Backend logs (last 20) ===' && "
    "docker logs xanesalon-backend-1 --tail=20 2>&1"
)
