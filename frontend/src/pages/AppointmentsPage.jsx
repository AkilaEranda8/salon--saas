...
export default function AppointmentsPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const payload = {
          ...
          phone: form.phone || '',
          staff_id: (() => {
            if (!multiBooking) return form.staff_id || null;
  ...
  // Available slots (shared schedule — multi-service also uses one start time)
  useEffect(() => {
    if (!showForm) {
      setAvailableSlots([]);
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            {isSuperAdmin && (
              ...
              <ApptSection title="Schedule" desc={`Date, time, and booking status · ${bookingDurationMinutes} min total`} dark={isDark}>
                ...
                {!editItem && !bookingUsesPackage && (
                  <FormGroup label="Custom amount (optional)">
                    <Input
                      type="number"
                      value={form.amount || ''}
