...
# Use mysql client inside the DB container
cmd = (
    "docker exec xanesalon-db-1 mysql -u root -prootpass zanesalon -e "
    "\"SELECT id, tenant_id, branch_id, smtp_host, smtp_port, smtp_user, smtp_from, "
    "LEFT(IFNULL(smtp_pass,''),8) as smtp_pass_prefix, LENGTH(IFNULL(smtp_pass,'')) as smtp_pass_len "
