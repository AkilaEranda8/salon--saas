  ...
  try {
    ...
    const { count, rows } = await Payment.findAndCountAll({
      ...
      include: [
        ...
        { model: Appointment, as: 'appointment', attributes: ['id', 'date', 'time', 'status', 'phone', 'customer_name'] },
    ...
    await enrichPaymentsForView(data);
...
const getOne = async (req, res) => {
  try {
    ...
    const name = displayCustomerName(json);
    if (name) json.customer_name = name;
    const payload = attachPaymentCommissionTotals(json);
    await enrichPaymentsForView(payload);
...
const create = async (req, res) => {
  ...
  try {
    ...
    await recordCommissionTransactions({
      ...
      managerStaffId: hasFranchiseCommission(req.tenant) ? manager_staff_id : null,
      managerAmount: hasFranchiseCommission(req.tenant) ? manager_commission_amount : 0,
      managerPercent: hasFranchiseCommission(req.tenant) ? manager_override_percent : null,
      managerBreakdown: hasFranchiseCommission(req.tenant) ? manager_commission_breakdown : null,
    ...
    try {
      ...
      const json = (created || payment).toJSON();
      const name = displayCustomerName(json);
      if (name) json.customer_name = name;
      const payload = attachPaymentCommissionTotals(json);
      await enrichPaymentsForView(payload);
      ...
      console.error('payment create enrich failed:', viewErr);
