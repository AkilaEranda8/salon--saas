...
class _AddApptSheetState extends State<_AddApptSheet> {
  ...
  Future<void> _save() async {
    ...
    String saveDate = _date;
    String saveTime = _time;
    if (_multiBooking) {
      _syncAssignments();
      for (final id in _orderedServiceIds()) {
        final a = _serviceAssignments[id] ?? {};
      ...
      final first = _serviceAssignments[_orderedServiceIds().first] ?? {};
      saveDate = '${first['date'] ?? ''}';
      saveTime = '${first['time'] ?? ''}';
      serviceStaff = _orderedServiceIds().map((id) {
        final a = _serviceAssignments[id] ?? {};
        final staff = (a['staff_id'] ?? '').toString().trim();
    ...
    final ok = await app.saveAppointment(
      ...
      baseNotes: pkgNote,
      status: '',
      amountOverride: amountOverride.isEmpty ? null : amountOverride,
      bookingItems: null,
                ...
                children: [
                  ...
                  WalkInServiceDropdownSection(
                    ...
                    helperText: _multiBooking
                        ? 'Pick services — set time for each below.'
                        : 'Pick one service.',
                    accentColor: _cMid,
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            DropdownButtonFormField<String?>(
                              ...
                              isExpanded: true,
                              decoration: _deco('Staff (optional)', Icons.badge_outlined),
                              items: [
                                const DropdownMenuItem<String?>(
                                    value: null, child: Text('Use main staff')),
                  ...
                  // Date + Time row (single service mode)
                  if (!_multiBooking)
                  Row(children: [
                    // Date
                    Expanded(
                  ...
                  // Staff + Branch row (main staff fallback in multi mode)
                  Row(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Expanded(
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            _label(_multiBooking ? 'MAIN STAFF' : 'STAFF'),
                            DropdownButtonFormField<String>(
                              key: ValueKey(
